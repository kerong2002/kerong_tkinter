...
def game_new_set():
    ...
    for y in range(1,game_Y_size-1):
        for x in range(1,game_X_size-1):
...
'''=====================<計時器>======================'''
def change_time():
    global counter,btn,t,snake,snake_size,game_over,score,snake_speed,ok,game_over_Label,body_eat,counter,reset,old_list_save,active_x,active_y,run_direction
    ...
    if(counter%15==0):
        ...
        if(run_direction==0):
            ...
            run_direction=0
    # if(active_y==0):
    #     if (Food_chose[1] > snake[0][1]):
    #             active_x = 0
    #             active_y = 1
    #     elif (Food_chose[1] < snake[0][1]):
    #             active_x = 0
    #             active_y = -1
    #     elif (Food_chose[1] ==snake[0][1]):
    #         if (Food_chose[0] > snake[0][0]):
    #                 active_x = 1
    #                 active_y = 0
    #         if (Food_chose[0] < snake[0][0]):
    #                 active_x = -1
    ...
    if((snake[0][0]>=15 or snake[0][0]<0 or snake[0][1]>=15 or snake[0][1]<0 or body_eat==1 ) and No_Died==0):
        ...
        pause()                                                 #停止
        return
    btn[snake[1][1]][snake[1][0]].config(bg='black')                                   #蛇身
    btn[snake[len(snake)-1][1]][snake[len(snake)-1][0]].config(bg='#008216')           #蛇尾染色
    btn[snake[0][1]][snake[0][0]].config(bg='#00a916')                                 #蛇頭染色
