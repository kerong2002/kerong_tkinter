...
window.bind('<Shift-Up>',lambda event: hack())                                        #外掛
'''=========================<數字顏色>============================'''
text_color={
' ':'black',
'💣':'#f712fc',
...
'''================<搜尋空白演算法>============================'''
def  flood_fill(y,x):
    global bomb_number_cnt,savex,savey
    if(x < game_X_size-1):
...
begin=0
def play(event,x,y):
    global t,begin,time_end,appear_flag,bomb_number_cnt,can_paly,trigger,is_win
    trigger = True
    if(is_win==True):
        return
        ...
        if(chess[y][x]!='💣'):
            ...
            if(chess[y][x]==' '):
                ...
                for i in range(len(savex)):
                    if (appear_chess[savey[i]][savex[i]] == ''):
                        play(True,savex[i],savey[i])
