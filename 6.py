...
def flash_change_time(y, x):
    global flash_counter,flash_t,big_small_time
...
def button_flash(y, x):
    global button_cnt,button_t,button_flash_speed,can_play
    ...
    if(button_cnt==big_small_time):
        ...
        can_play=True
        return
    button_t= Timer(button_flash_speed, button_flash, [1, 2])
    button_t.start()
def double(x):
    ...
    Small_game.config(state='disable')
    GO_game.config(state='disable')
    big_small_time = randint(16, 25)
    button_t = Timer(0.1, button_flash, [1, 2])
    button_t.start()
def play(event):
...
win_money_total=Label(frame1,text='%7d'%all_title_win_money,bg='#e1971c',fg='red',font=('arial',20)).place(x=190,y=145)
png_cnt=0
...
GO_game=Button(frame1,text="GO",width=5,height=2,bg='#ee2020')
GO_game.bind("<Button-1>",play)#左鍵
